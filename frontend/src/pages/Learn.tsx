...
export function Learn({ onReturnDashboard }: { onReturnDashboard: () => void }) {
  const { recommendations, selection, loadActivity, submitAnswer, online, curriculum } = useLearning();
  ...
  if (stage === "check") return <div className="learning-stage">{curriculum && <p className="curriculum-breadcrumb">Class {curriculum.class_level} <span>›</span> {curriculum.subject_name} <span>›</span> {curriculum.chapter_title}</p>}<div className="learn-topline"><Badge tone="cyan">Adaptive checkpoint</Badge><span>{humanize(selection.selection_type)}</span></div>{error && <div className="inline-error" role="alert">{error}</div>}<QuizCard question={selection.question} onSubmit={(answer, hints, elapsed) => void submit(answer, hints, elapsed)} submitting={submitting} /></div>;
  if (!content) return <ErrorState message="No recommendation content is available yet." onRetry={() => setStage("check")} />;
  return <div className="learning-stage">{curriculum && <p className="curriculum-breadcrumb">Class {curriculum.class_level} <span>›</span> {curriculum.subject_name} <span>›</span> {content.activity.curriculum_context.chapter_title}</p>}<div className="learn-topline"><div><Badge tone={recommendation?.adaptation_path === "misconception_remediation" ? "amber" : "indigo"}>{recommendation ? humanize(recommendation.adaptation_path) : "Guided lesson"}</Badge>{source === "cache" && <Badge tone="slate">Saved offline</Badge>}</div><span>{content.content.estimated_minutes ?? 5} min</span></div>{recommendation?.adaptation_path === "misconception_remediation" && <div className="supportive-banner"><strong>Let’s fix one common mix-up</strong><p>This short visual is here to make the idea clearer—not to mark a mistake.</p></div>}<ActivityRenderer content={content.content} /><div className="lesson-footer"><div><strong>Ready to try it?</strong><p>A quick check helps choose your next best activity.</p></div><button className="button-primary" onClick={() => setStage("check")}>Start quick check</button></div></div>;
}
